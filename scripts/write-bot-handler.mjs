  ...
  try {
    ...
    if (data === "buy") {
      ...
      if (products.length === 0) {
        await bot.editMessageText(chatId, messageId, "❌ فعلاً محصولی موجود نیست.", {
    ...
    if (data === "my_accounts") {
      ...
      if (orders.length === 0) {
        await bot.editMessageText(chatId, messageId, "📭 هنوز اکانتی نداری.", {
          reply_markup: backToMainKeyboard(),
        });
        await bot.answerCallbackQuery(cb.id);
        return;
    ...
    if (data === "help") {
      const text =
        ...
        "📱 لینک رو توی <b>V2Box</b> یا <b>Hiddify</b> import کن.";
      await bot.editMessageText(chatId, messageId, text, {
        reply_markup: backToMainKeyboard(),
      });
      await bot.answerCallbackQuery(cb.id);
    ...
    if (data === "admin_panel" && isAdmin) {
      await bot.editMessageText(chatId, messageId, "🔧 <b>پنل ادمین</b>", {
        reply_markup: adminMenuKeyboard(),
      });
      await bot.answerCallbackQuery(cb.id);
      return;
    ...
    if (data === "admin_pending" && isAdmin) {
      ...
      if (orders.length === 0) {
        await bot.editMessageText(chatId, messageId, "📭 سفارش در انتظاری نداری.", {
          reply_markup: adminMenuKeyboard(),
        });
        await bot.answerCallbackQuery(cb.id);
        return;
...
async function handleReceiptPhoto(
  ...
  if (!state || state.state !== "awaiting_receipt") {
    await bot.sendMessage(
      msg.chat.id,
      "⚠️ اول یه پلن انتخاب کن و فیش رو از اون مسیر بفرست.\\n/start",
      { reply_markup: backToMainKeyboard() }
    );
    return;
  }
  ...
  await bot.sendMessage(
    msg.chat.id,
    "✅ فیش شما دریافت شد!\\n\\n⏳ در انتظار تایید ادمین...\\n\\nبعد از تایید، لینک اتصال برات ارسال می‌شه.",
    { reply_markup: backToMainKeyboard() }
  );
...
// ============================================================
// Order approval — direct DB + node task (no HTTP)
// ============================================================
async function approveOrder(
  ...
  if (!order) {
    await bot.editMessageText(adminChatId, adminMessageId, "❌ سفارش پیدا نشد", {
      reply_markup: adminMenuKeyboard(),
    });
    return;
  }
  ...
  if (order.status === "completed") {
    await bot.editMessageText(adminChatId, adminMessageId, "⚠️ این سفارش قبلاً تایید شده", {
      reply_markup: adminMenuKeyboard(),
    });
    return;
  }
  ...
  try {
    ...
    if (!syncRes.ok || !syncRes.output || !syncRes.output.stdout.includes("SYNC_DONE")) {
      ...
      // Continue anyway — user will be in D1
    }
...
async function rejectOrder(
  ...
  await bot.editMessageText(adminChatId, adminMessageId, "❌ سفارش رد شد", {
    reply_markup: adminMenuKeyboard(),
  });
}
